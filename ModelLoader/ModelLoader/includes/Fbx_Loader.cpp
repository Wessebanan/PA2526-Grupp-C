			...
			for (unsigned int deformer_index = 0; deformer_index < num_deformers; ++deformer_index)
			{
				...
				for (unsigned int cluster_index = 0; cluster_index < num_of_clusters; ++cluster_index)
				{
					...
					if (curr_anim_stack)
					{
						...
						ModelLoader::KeyFrame** curr_anim = &skeleton->joints[curr_joint_index].mAnimation;
						...
						for (FbxLongLong i = start.GetFrameCount(FbxTime::eFrames24); i <= end.GetFrameCount(FbxTime::eFrames24); ++i)
						{
							...
							*curr_anim = new ModelLoader::KeyFrame();
							(*curr_anim)->mFrameNum = i;

							ModelLoader::KeyFrame newSystemKeyFrame;
...
HRESULT ModelLoader::LoadFBX(const std::string& fileName, std::vector<DirectX::XMFLOAT3>* pOutVertexPosVector, std::vector<int>* pOutIndexVector,
	std::vector<DirectX::XMFLOAT3>* pOutNormalVector, std::vector<DirectX::XMFLOAT2>* pOutUVVector, ModelLoader::Skeleton* pOutSkeleton, std::vector<ControlPointInfo>* pOutCPInfoVector)
{
